  ...
  return {
    ...
    template: '<div class="multiple-date-picker">'+
            ...
            '<div class="picker-days-week-row">'+
              '<div ng-class="{\'active\': weekdayBatchSelectable }" class="weekday-header text-center" ng-repeat="day in daysOfWeek" ng-click=selectWeekdayInRange($index)>{{day}}</div>'+
            '</div>'+
    ...
    link: function(scope, elm, attrs, ctrl){
      ...
      //default values
      scope.maxDaysSelectable = scope.maxDaysSelectable || null;
      scope.weekdayBatchSelectable = scope.weekdayBatchSelectable || false;
      console.log('weekdayBatchSelectable',scope.weekdayBatchSelectable);
      ...
      scope.toggleDay = function(event, momentDate){
        ...
        if(momentDate.selectable && !prevented){
          ...
          if(momentDate.selected) {
            if (scope.maxDaysSelectable != null) {

              if (scope.convertedDaysSelected.length >= scope.maxDaysSelectable) {
              // overwrite last selected date
                scope.convertedDaysSelected[length-1].selected = false;
                scope.convertedDaysSelected[length-1] = momentDate;
              } else {
